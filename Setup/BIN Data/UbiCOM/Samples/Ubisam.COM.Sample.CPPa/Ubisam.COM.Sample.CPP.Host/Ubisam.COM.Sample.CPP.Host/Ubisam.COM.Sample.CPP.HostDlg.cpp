...
// Ubisam.COM.Sample.CPP.HostDlg.cpp : 구현 파일
//

#include "stdafx.h"
#include "Ubisam.COM.Sample.CPP.Host.h"
#include "Ubisam.COM.Sample.CPP.HostDlg.h"
...
// CUbisamCOMSampleCPPHostDlg 대화 상자




CUbisamCOMSampleCPPHostDlg::CUbisamCOMSampleCPPHostDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CUbisamCOMSampleCPPHostDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
	this->m_uLastReceivedS1F1SystemByte = 0;
}

void CUbisamCOMSampleCPPHostDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CUbisamCOMSampleCPPHostDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_EXIT, &CUbisamCOMSampleCPPHostDlg::OnBnClickedExit)
	ON_BN_CLICKED(IDC_CLEAR_LOG, &CUbisamCOMSampleCPPHostDlg::OnBnClickedClearLog)
	ON_BN_CLICKED(IDC_INIT_CONFIG, &CUbisamCOMSampleCPPHostDlg::OnBnClickedInitConfig)
	ON_BN_CLICKED(IDC_INIT_UCF_FILE, &CUbisamCOMSampleCPPHostDlg::OnBnClickedInitUcfFile)
	ON_BN_CLICKED(IDC_CONNECTION_OPEN, &CUbisamCOMSampleCPPHostDlg::OnBnClickedConnectionOpen)
	ON_BN_CLICKED(IDC_CONNECTION_CLOSE, &CUbisamCOMSampleCPPHostDlg::OnBnClickedConnectionClose)
	ON_BN_CLICKED(IDC_SEND_S1F1, &CUbisamCOMSampleCPPHostDlg::OnBnClickedSendS1f1)
	ON_BN_CLICKED(IDC_REPLY_S1F2, &CUbisamCOMSampleCPPHostDlg::OnBnClickedReplyS1f2)
	ON_BN_CLICKED(IDC_BROWSE_LOG_PATH, &CUbisamCOMSampleCPPHostDlg::OnBnClickedBrowseLogPath)
	ON_BN_CLICKED(IDC_BROWSE_UMD_FILE, &CUbisamCOMSampleCPPHostDlg::OnBnClickedBrowseUmdFile)
	ON_BN_CLICKED(IDC_BROWSE_UCF_FILE, &CUbisamCOMSampleCPPHostDlg::OnBnClickedBrowseUcfFile)
	ON_BN_CLICKED(IDC_SEND_S2F41, &CUbisamCOMSampleCPPHostDlg::OnBnClickedSendS2f41)
END_MESSAGE_MAP()


// CUbisamCOMSampleCPPHostDlg 메시지 처리기

BOOL CUbisamCOMSampleCPPHostDlg::OnInitDialog()
{
	...
	this->m_radEQP.SetCheck(0);
	this->m_radHost.SetCheck(1);
	this->m_txtDriverNameConfig.SetWindowText(_T("CPP_TEST_HOST"));
	...
	this->m_radPassive.SetCheck(0);
	this->m_radActive.SetCheck(1);
...
void CUbisamCOMSampleCPPHostDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CUbisamCOMSampleCPPHostDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void CUbisamCOMSampleCPPHostDlg::OnReceivedInvalidPrimaryMessageEventHandler(UbiCom::CPP::SECSMessageValidationError reason, UbiCom::CPP::SECSMessage* message)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnReceivedInvalidSecondaryMessageEventHandler(UbiCom::CPP::SECSMessageValidationError reason, UbiCom::CPP::SECSMessage* primaryMessage, UbiCom::CPP::SECSMessage* secondaryMessage)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSentControlMessageEventHandler(UbiCom::CPP::SECSMessage* message)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSentSECSMessageEventHandler(UbiCom::CPP::SECSMessage* message)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnReceivedControlMessageEventHandler(UbiCom::CPP::SECSMessage* message)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnReceivedPrimaryMessageEventHandler(UbiCom::CPP::SECSMessage* primaryMessage)
{
	CString text;
	...
	if (primaryMessage->AutoReply == false && primaryMessage->WaitBit == true)
	{
		...
		else if (primaryMessage->Stream == 6 && primaryMessage->Function == 11)
		{
			AnalyzeS6F11(primaryMessage);
		}
	}
}

void CUbisamCOMSampleCPPHostDlg::OnReceivedSecondaryMessageEventHandler(UbiCom::CPP::SECSMessage* primaryMessage, UbiCom::CPP::SECSMessage* secondaryMessage)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnReceivedUnknownMessageEventHandler(UbiCom::CPP::SECSMessage* message)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnTimeoutEventHandler(UbiCom::CPP::SECSTimeoutType timeoutType)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnT3TimeoutEventHandler(UbiCom::CPP::SECSMessage* message)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSECSConnectedEventHandler(tstring& ipAddress, int portNo)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSECSDisconnectedEventHandler(tstring& ipAddress, int portNo)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSECSSelectedEventHandler(tstring& ipAddress, int portNo)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSECSDeselectedEventHandler(tstring& ipAddress, int portNo)
{
	CString text;
...
void CUbisamCOMSampleCPPHostDlg::OnSECS1WriteLogEventHandler(UbiCom::CPP::SECSLogLevel logLevel, tstring& logText)
{
	AddLog(ConvertToCString(logText), false);
}

void CUbisamCOMSampleCPPHostDlg::OnSECS2WriteLogEventHandler(UbiCom::CPP::SECSLogLevel logLevel, tstring& logText)
{
	AddLog(ConvertToCString(logText), false);
}

CString CUbisamCOMSampleCPPHostDlg::ConvertToCString(UbiCom::CPP::SECSLogLevel logLevel)
{
	CString result;
...
CString CUbisamCOMSampleCPPHostDlg::ConvertToCString(tstring& str)
{
	CString result;
...
CString CUbisamCOMSampleCPPHostDlg::ConvertToCString(UbiCom::CPP::SECSTimeoutType timeoutType)
{
	CString result;
...
CString CUbisamCOMSampleCPPHostDlg::ConvertToCString(UbiCom::CPP::SECSMessageValidationError error)
{
	CString result;
...
CString CUbisamCOMSampleCPPHostDlg::ConvertToCString(UbiCom::CPP::SECSDriverError error)
{
	CString result;
...
CString CUbisamCOMSampleCPPHostDlg::ConvertToCString(UbiCom::CPP::SECSMessageError error)
{
	CString result;
...
void CUbisamCOMSampleCPPHostDlg::AddLog(CString& message, bool withoutTime)
{
	CString text;
	SYSTEMTIME st;
	CString strAll;
...
CString CUbisamCOMSampleCPPHostDlg::GetContainingFolder(CString &filePath)
{
	CString result;
...
void CUbisamCOMSampleCPPHostDlg::ChangeUMDFilePath(CString& filePath)
{
	TCHAR * szFileName;
	TCHAR * szFileExt;
	...
	if(filePath != "")
	{
		...
		strTitle.Format(_T("Ubisam.COM.Sample.CPP.Host - %s"), filePath);
		SetWindowText(strTitle);
	}
	else
	{
		SetWindowText(_T("Ubisam.COM.Sample.CPP.Host"));
	}
...
void CUbisamCOMSampleCPPHostDlg::ChangeUCFFilePath(CString& filePath)
{
	TCHAR * szFileName;
	TCHAR * szFileExt;
	CString strFileNameWithExt;
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedInitConfig()
{
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedInitUcfFile()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedClearLog()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedExit()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedConnectionOpen()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedConnectionClose()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedSendS1f1()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedReplyS1f2()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UbiCom::CPP::SECSMessageError success;
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedBrowseLogPath()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedBrowseUmdFile()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedBrowseUcfFile()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CUbisamCOMSampleCPPHostDlg::OnBnClickedSendS2f41()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	...
	if (m_pDriver != nullptr)
	{
		message = m_pDriver->GetSECSMessage(2, 41);
...
*/
		message->AddListChild(_T(""), 2);
			message->AddChild(_T("RCMD"), UbiCom::CPP::SECSItemFormat_A, _T("PERMIT"));
			message->AddListChild(_T("CPCOUNT"), 3);
				message->AddListChild(_T(""), 2);
					message->AddChild(_T("CPNAME"), UbiCom::CPP::SECSItemFormat_A, _T("CONFIRMFLAG"));
					message->AddChild(_T("CPVAL"), UbiCom::CPP::SECSItemFormat_A, _T("Y"));
				message->AddListChild(_T(""), 2);
					message->AddChild(_T("CPNAME"), UbiCom::CPP::SECSItemFormat_A, _T("MODELID"));
					message->AddChild(_T("CPVAL"), UbiCom::CPP::SECSItemFormat_A, _T("MID11"));
				message->AddListChild(_T(""), 2);
					message->AddChild(_T("CPNAME"), UbiCom::CPP::SECSItemFormat_A, _T("OPID"));
					message->AddChild(_T("CPVAL"), UbiCom::CPP::SECSItemFormat_A, _T("11231"));
		...
		if (success != UbiCom::CPP::SECSMessageError_Ok)
		{
			temp.Format(_T("SendS2F41 Fail: Reason=%s"), ConvertToCString(success));
		}
		else
		{
			temp.Format(_T("SendS2F41 Success"));
		}
...
void CUbisamCOMSampleCPPHostDlg::AnalyzeS6F11(UbiCom::CPP::SECSMessage* message)
{
	UbiCom::CPP::SECSMessage* secondaryMessage;
	UbiCom::CPP::SECSMessageError success;
	CString temp;
	...
	if (message != nullptr)
	{
		...
		UbiCom::CPP::SECSItem* dataidItem;
		UbiCom::CPP::SECSItem* ceidItem;
		
		UbiCom::CPP::SECSItemCollection* rptIDCollection;
		UbiCom::CPP::SECSItemCollection* rptCollection;

		UbiCom::CPP::SECSItem* rptidItem;
		UbiCom::CPP::SECSItemCollection* vCountCollection;
...
>
		...
		*/
		if (message->Items != nullptr && message->Items->GetCount() == 1)
		{
			...
			if (rootCollection != nullptr && rootCollection->GetCount() == 3)
			{
				dataid = ConvertSECSItemValueToCString(rootCollection->GetItem(0));
				ceid = ConvertSECSItemValueToCString(rootCollection->GetItem(1));
				rptIDCollection = rootCollection->GetItem(2)->SubItem;

				log.AppendFormat(_T("DATAID=%s, CEID=%s"), dataid, ceid);

				if (rptIDCollection != nullptr)
				{
					for (int i = 0; i < rptIDCollection->GetCount(); i++)
					{
						rptCollection = rptIDCollection->GetItem(i)->SubItem;

						if (rptCollection != nullptr && rptCollection->GetCount() == 2)
						{
							rptid = ConvertSECSItemValueToCString(rptCollection->GetItem(0));
							
							log.AppendFormat(_T("\r\n    RPTID=%s"), rptid);

							vCountCollection = rptCollection->GetItem(1)->SubItem;

							if (vCountCollection != nullptr)
							{
								RetrievingS6F11V(log, 2, vCountCollection);
							}
						}
					}
				}
		...
		if (this->m_pDriver != nullptr)
		{
			secondaryMessage = this->m_pDriver->GetSECSMessage(6, 12);

			if (secondaryMessage != nullptr)
			{
				secondaryMessage->AddChild(_T("ACKC6"), UbiCom::CPP::SECSItemFormat_B, (uint8_t)0);

				success = m_pDriver->ReplySECSMessage(message, secondaryMessage);

				if (success != UbiCom::CPP::SECSMessageError_Ok)
				{
					temp.Format(_T("ReplyS6F12 Fail: Reason=%s"), ConvertToCString(success));
				}
				else
				{
					temp.Format(_T("ReplyS6F12 Success"));
				}
...
CString CUbisamCOMSampleCPPHostDlg::ConvertSECSItemValueToCString(UbiCom::CPP::SECSItem* item)
{
	CString result(_T(""));
	...
	if (item != nullptr)
	{
		...
		case UbiCom::CPP::SECSItemFormat_I4:
			if (item->Length == 1)
			{
				result.Format(_T("%ld"), item->GetValueI4());
			}
			else
			{
				...
				if (item->Length > 0 && src != nullptr)
				{
					for (int i = 0; i < item->Length; i++)
					{
						result.AppendFormat(_T("%ld "), src[i]);
					}
				}
			}
			break;
		case UbiCom::CPP::SECSItemFormat_I8:
			if (item->Length == 1)
			{
				result.Format(_T("%lld"), item->GetValueI8());
			}
			else
			{
				...
				if (item->Length > 0 && src != nullptr)
				{
					for (int i = 0; i < item->Length; i++)
					{
						result.AppendFormat(_T("%lld "), src[i]);
					}
				}
			}
...
void CUbisamCOMSampleCPPHostDlg::RetrievingS6F11V(CString& log, int logDepth, UbiCom::CPP::SECSItemCollection* collection)
{
	UbiCom::CPP::SECSItem* item;
	CString itemValue;
	...
	if (collection != nullptr)
	{
		for (int i = 0; i < collection->GetCount(); i++)
		{
			...
			if (item != nullptr)
			{
				...
				for (int j = 0; j < logDepth; j++)
				{
					log.Append(_T("    "));
				}

				if (item->Format == UbiCom::CPP::SECSItemFormat_L)
				{
					log.AppendFormat(_T("V_%d List"), i);
					RetrievingS6F11V(log, logDepth + 1, item->SubItem);
				}
				else
				{
					itemValue = ConvertSECSItemValueToCString(item);
					
					log.AppendFormat(_T("V_%d Value=%s"), i, itemValue);
				}
			}
		}
	}
